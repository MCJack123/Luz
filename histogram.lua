                ...
                if ok then
                    ...
                    for _, v in ipairs(res) do
                        ...
                        if freq[v.text] or v.type == "keyword" or v.type == "operator" or v.type == "constant" then
                            ...
                            if v.type == "name" then
                                for c in v.text:gmatch "." do namefreq[b64lut[c]] = (namefreq[b64lut[c]] or 0) + 1 end
                                namefreq[63] = (namefreq[63] or 0) + 1
                            end
                        end
                    end
                end
...
file.close()
local map, lengths, tree = maketree(out)
...
file.close()
local nameout = {}
for i = 0, 63 do if not namefreq[i] then error("Missing character " .. b64str:sub(i+1, i+1) .. ", please try again") end nameout[i+1] = {i, namefreq[i]} end
map, lengths, tree = maketree(nameout)
file = fs.open(shell.resolve("name_encode_map.lua"), "w")
file.write(("-- AUTOGENERATED\nreturn %s"):format(textutils.serialize(map)))
file.close()
file = fs.open(shell.resolve("name_decode_tree.lua"), "w")
file.write(("-- AUTOGENERATED\nreturn %s"):format(textutils.serialize(tree, {compact = true}):gsub(",}", "}")))
file.close()
