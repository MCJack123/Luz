...
file.close()
local lut = setmetatable({[":end"] = 256}, {__index = function(_, v) error(v) end})
for i = 0, 255 do lut[i] = i end
for i = 0, 29 do lut[":repeat" .. i] = i + 257 end
local toklist = {}
for k in pairs(freq) do if type(k) == "string" and not k:match "^:repeat" and k ~= ":end" then toklist[#toklist+1] = k end end
table.sort(toklist)
for i, v in ipairs(toklist) do lut[v] = i+286 end
